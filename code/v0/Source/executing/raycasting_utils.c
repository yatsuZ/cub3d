...
{
	all->file->camera.x = 2 * i / (double)all->mini->sizex - 1;
...
void	perform_dda(t_all_data *all)
{
	...
	{
		all->file->step.x = -1; 
		all->file->sidedist.x = (all->world->spawn.x - all->file->map.x) * \
				all->file->delta.x;
	}
	else
	{
		all->file->step.x = 1;
		all->file->sidedist.x = (all->file->map.x  + 1.0 - all->world->spawn.x) * \
				all->file->delta.x; 
	}
	if (all->file->dir.y < 0)
	{
		all->file->step.y = -1;
		all->file->sidedist.y = (all->world->spawn.y - all->file->map.y) * \
				all->file->delta.y;
	}
	else
	{
		all->file->step.y = 1;
		all->file->sidedist.y = (all->file->map.y  + 1.0 - all->world->spawn.y) * \
				all->file->delta.y;
	}
}
